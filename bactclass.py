...
def cross_validate(classifier, X, Y, fold):
    ...
    print("Recall:               %0.2f (sigma = %0.3f)" % (scores.mean()*100, scores.std()*100))
    scores = cross_val_score(classifier, X, Y, cv=fold, scoring="f1")
    print("F1 Score:             %0.2f (sigma = %0.3f)" % (scores.mean()*100, scores.std()*100))
    scores = cross_val_score(classifier, X, Y, cv=fold, scoring="accuracy")
    print("Accuracy:             %0.2f (sigma = %0.3f)" % (scores.mean()*100, scores.std()*100))
    scores = cross_val_score(classifier, X, Y, cv=fold, scoring="roc_auc")
    print("Area Under ROC Curve: %0.2f (sigma = %0.3f)" % (scores.mean()*100, scores.std()*100))
    scores = cross_val_score(classifier, X, Y, cv=fold, scoring="jaccard")
    print("Jaccard Score:        %0.2f (sigma = %0.3f)" % (scores.mean()*100, scores.std()*100))
    scores = cross_val_score(classifier, X, Y, cv=fold, scoring="r2")
    print("R-square:             %0.2f (sigma = %0.3f)" % (scores.mean()*100, scores.std()*100))
